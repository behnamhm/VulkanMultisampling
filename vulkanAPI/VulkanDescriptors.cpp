...
void VulkanDescriptors::createDescriptorSetLayout()
{
	...
	VkResult result = vkCreateDescriptorSetLayout(vulkanDevice.logicalDevice, &layoutCreateInfo, nullptr, &descriptorSetLayout);
	...
	// Create Descriptor Set Layout
	result = vkCreateDescriptorSetLayout(vulkanDevice.logicalDevice, &textureLayoutCreateInfo, nullptr, &samplerSetLayout);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Set Layout!");
	}
	...
	// Create Descriptor Set Layout
	result = vkCreateDescriptorSetLayout(vulkanDevice.logicalDevice, &inputLayoutCreateInfo, nullptr, &inputSetLayout);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Set Layout!");
	}
...
void VulkanDescriptors::createDescriptorPool(uint32_t swapChainSize, std::vector<VkImageView>& depthBufferImageView,
											 std::vector<VkImageView>& colourBufferImageView)
{
	...
	vpPoolSize.descriptorCount = static_cast<uint32_t>(vulkanDevice.vpUniformBuffer.size());
	...
	poolCreateInfo.maxSets = static_cast<uint32_t>(swapChainSize);					// Maximum number of Descriptor Sets that can be created from pool
	poolCreateInfo.poolSizeCount = static_cast<uint32_t>(descriptorPoolSizes.size());		// Amount of Pool Sizes being passed
	poolCreateInfo.pPoolSizes = descriptorPoolSizes.data();									// Pool Sizes to create pool with

	// Create Descriptor Pool
	VkResult result = vkCreateDescriptorPool(vulkanDevice.logicalDevice, &poolCreateInfo, nullptr, &descriptorPool);
	if (result != VK_SUCCESS)
	{
	...
	result = vkCreateDescriptorPool(vulkanDevice.logicalDevice, &samplerPoolCreateInfo, nullptr, &samplerDescriptorPool);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Pool!");
	}
	...
	inputPoolCreateInfo.maxSets = swapChainSize;
	inputPoolCreateInfo.poolSizeCount = static_cast<uint32_t>(inputPoolSizes.size());
	inputPoolCreateInfo.pPoolSizes = inputPoolSizes.data();

	result = vkCreateDescriptorPool(vulkanDevice.logicalDevice, &inputPoolCreateInfo, nullptr, &inputDescriptorPool);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Pool!");
	}
...
void VulkanDescriptors::createDescriptorSets(uint32_t swapChainSize)
{
	// Resize Descriptor Set list so one for every buffer
	descriptorSets.resize(swapChainSize);

	std::vector<VkDescriptorSetLayout> setLayouts(swapChainSize, descriptorSetLayout);

	// Descriptor Set Allocation Info
	VkDescriptorSetAllocateInfo setAllocInfo = {};
	setAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	setAllocInfo.descriptorPool = descriptorPool;									// Pool to allocate Descriptor Set from
	setAllocInfo.descriptorSetCount = static_cast<uint32_t>(swapChainSize);// Number of sets to allocate
	setAllocInfo.pSetLayouts = setLayouts.data();									// Layouts to use to allocate sets (1:1 relationship)

	// Allocate descriptor sets (multiple)
	VkResult result = vkAllocateDescriptorSets(vulkanDevice.logicalDevice, &setAllocInfo, descriptorSets.data());
	if (result != VK_SUCCESS)
	{
	...
	for (size_t i = 0; i < swapChainSize; i++)
	{
		...
		vpBufferInfo.buffer = vulkanDevice.vpUniformBuffer[i];		// Buffer to get data from
		...
		vkUpdateDescriptorSets(vulkanDevice.logicalDevice, static_cast<uint32_t>(setWrites.size()), setWrites.data(),
			0, nullptr);
	}
}
...
void VulkanDescriptors::createInputDescriptorSets(uint32_t swapChainSize, std::vector<VkImageView>& depthBufferImageView,
													std::vector<VkImageView>& colourBufferImageView)
{
	// Resize array to hold descriptor set for each swap chain image
	inputDescriptorSets.resize(swapChainSize);

	// Fill array of layouts ready for set creation
	std::vector<VkDescriptorSetLayout> setLayouts(swapChainSize, inputSetLayout);

	// Input Attachment Descriptor Set Allocation Info
	VkDescriptorSetAllocateInfo setAllocInfo = {};
	setAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	setAllocInfo.descriptorPool = inputDescriptorPool;
	setAllocInfo.descriptorSetCount = static_cast<uint32_t>(swapChainSize);
	setAllocInfo.pSetLayouts = setLayouts.data();

	// Allocate Descriptor Sets
	VkResult result = vkAllocateDescriptorSets(vulkanDevice.logicalDevice, &setAllocInfo, inputDescriptorSets.data());
	if (result != VK_SUCCESS)
	{
	...
	for (size_t i = 0; i < swapChainSize; i++)
	{
		...
		vkUpdateDescriptorSets(vulkanDevice.logicalDevice, static_cast<uint32_t>(setWrites.size()), setWrites.data(), 0, nullptr);
	}
}

void VulkanDescriptors::cleanup()
{
